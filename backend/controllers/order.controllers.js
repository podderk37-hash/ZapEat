...
export const placeOrder = async (req, res) => {
    try {
        ...
        cartItems.forEach(item => {
            ...
            if (!shopId) {
                console.warn("❌ Missing shopId for:", item.name);
    ...
    try {
        ...
        if (status == "out of delivery" && !shopOrder.assignment) {
            ...
            if (candidates.length == 0) {
                await order.save()
                return res.json({ message: "order status updated but there no available delivery boys" })
            }
...
export const sendDeliveryOtp = async (req, res) => {
    try {
        ...
        if (!order || !shopOrder) {
            return res.status(400).json({ message: " Enetr valid order/shopOrderId" })
        }
